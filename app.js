...
document.getElementById('hdrStats').innerHTML = `
  <span class="header-pill">${meta.partic}% participación</span>
...
function getMarkerRadius(zoom) {
  // Zoom 6 (alejado) -> radio 3
  // Zoom 10 (medio) -> radio 5
  // Zoom 14+ (cercano) -> radio 8
  if (zoom <= 7) return 3;
  if (zoom <= 9) return 4;
  if (zoom <= 11) return 5;
...
function popupSV(h) {
  ...
  return `
    ...
    <div class="popup-meta">
      <div><div class="popup-meta-num">${num(h.hab_sv || h.hab)}</div><div class="popup-meta-label">habilitados</div></div>
      <div><div class="popup-meta-num">${num(h.emi_sv || h.emi)}</div><div class="popup-meta-label">emitidos</div></div>
      <div><div class="popup-meta-num">${(h.par_sv || h.par).toFixed(1)}%</div><div class="popup-meta-label">participación</div></div>
    </div>`;
}
...
function renderMarkers() {
  ...
  for (const r of f) {
    ...
    m.on('mouseout', function () { 
      if (!this.isPopupOpen()) this.setStyle({ 
        fillOpacity: .75, 
        weight: 1, 
        color: 'rgba(0,0,0,.12)',
        radius: radius
...
function updateFiltered(f) {
  ...
  const fV = order.map(p => ({ p, v: f.reduce((s, r) => s + (isSV ? (r['sv' + p] || 0) : (r.v[p] || 0)), 0) })).sort((a, b) => b.v - a.v);
  const fT = fV.reduce((s, x) => s + x.v, 0);
  const fH = f.reduce((s, r) => s + (isSV ? (r.hab_sv || r.hab) : r.hab), 0);
  const fE = f.reduce((s, r) => s + (isSV ? (r.emi_sv || r.emi) : r.emi), 0);
  el.innerHTML = `
...
function buildSb() {
  ...
  if (!isSV) {
    ...
    h += `<div class="sb-section">
      <div class="sb-label">Resultado nacional – Segunda vuelta</div>
      <div class="results-panel">
